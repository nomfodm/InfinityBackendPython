...
def mock_auth_service(mocker: MockerFixture) -> AuthService:
    mock_auth_service_class = mocker.patch(
        "application.use_cases.auth.register_user_and_login.AuthService"
    )
    ...
    fake_session_tokens.tokens = TokenPairResponse(
        ...
        refresh_split_token="uuid.refresh_token"
    )
    ...
    mock_auth_instance.generate_refresh_token.return_value = ("refresh_token", datetime.datetime.now(datetime.UTC),
                                                              "refresh_token_hash")
