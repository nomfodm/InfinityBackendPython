...
from domain.entities.wardrobe import WardrobeItem, SkinModel
...
class MinecraftProfileService:
    ...
    def build_profile_response(self, *, mc_profile: MinecraftProfile,
                               skin_wardrobe_item: WardrobeItem | None,
                               cape_wardrobe_item: WardrobeItem | None) -> ProfileResponse:
        textures_pr_value = {
        ...
        if mc_profile.active_skin_id is not None:
            ...
            if skin_wardrobe_item.texture.model == SkinModel.SLIM:
                textures_pr_value["textures"]["SKIN"]["metadata"] = {
                    "model": "slim"
                }
        ...
        textures_property = ProfileProperty(
            name="textures",
            value=textures_pr_value_b64,
            signature=signature
        )

        return ProfileResponse(
            id=mc_profile.uuid.hex,
            name=mc_profile.nickname.value,
